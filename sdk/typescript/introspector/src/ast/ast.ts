...
import type { DaggerDecorators } from "../utils/decorator"
import type { DeclarationsMap } from "./declarations"
import { isDeclarationOf } from "./declarations"
...
export class AST {
  ...
  public getSymbolOrThrow(node: ts.Node): ts.Symbol {
    ...
    if (!symbol) {
      throw new Error(`could not find symbol at ${AST.getNodePosition(node)}`)
    }
